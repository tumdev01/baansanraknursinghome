...
class ProfileManager {
	...
	public function save_profile() {
		...
		if ( isset( $state_data['fromRecent'] ) && $state_data['fromRecent'] ) {
			$recent_migrations = get_site_option( WPMDB_RECENT_MIGRATIONS_OPTION );
		...
		$this->http->end_ajax( [ 'id' => max( array_keys( $profiles ) ) ] );
	}
	...
	public function save_recent_migration() {
		...
		$existing_profiles = get_site_option( WPMDB_RECENT_MIGRATIONS_OPTION );
		...
		update_site_option( WPMDB_RECENT_MIGRATIONS_OPTION, $profiles );

		$this->http->end_ajax(
			[
				'date'     => $date,
				'id'       => max( array_keys( $profiles ) ),
				'profiles' => $this->assets->get_recent_migrations( $profiles ),
			]
		);
	}
	...
	public function remove_recent_migration() {
		...
		if ( empty( $existing_profiles ) ) {
			$this->http->end_ajax( new WP_Error( 'wpmdb_error', __( 'No recent migrations', 'wp-migrate-db' ) ) );
		...
		update_site_option( WPMDB_RECENT_MIGRATIONS_OPTION, $existing_profiles );

		$this->http->end_ajax( $state_data['id'] . ' Removed 👍' );
	}
