...
class ErrorLog {
	...
	public function __construct(
		...
		add_action( 'wpmdb_track_migration_error', array( $this, 'automatically_log_migration_error' ) );
	}
	...
	function load_error_log() {
		$this->error_log = get_site_option( WPMDB_ERROR_LOG_OPTION );
		...
		if ( false === $this->error_log && is_multisite() && is_network_admin() ) {
			$this->error_log = get_option( WPMDB_ERROR_LOG_OPTION );
			if ( false !== $this->error_log ) {
				update_site_option( WPMDB_ERROR_LOG_OPTION, $this->error_log );
	...
	/**
	 * Save an error, so it can be seen in the diagnostic info.
	 *
	 * @param string $wpmdb_error
	 * @param mixed  $additional_error_var
	 *
	 * @return void
	 * @throws DependencyException
	 * @throws NotFoundException
	 */
	public function log_error( $wpmdb_error, $additional_error_var = false ) {
		$state_data = WPMDBDI::getInstance()->get( MigrationStateManager::class )->set_post_data();

		// Oh dear, we really do have an issue!
	...
	function get_diagnostic_info() {
		...
		$mdb_plugins  = array();
		$plugin_title = WPMDB_PLUGIN_TITLE;
		if ( $GLOBALS['wpmdb_meta'][ WPMDB_CORE_SLUG ]['version'] ) {
			$mdb_plugins[ $plugin_title ] = $GLOBALS['wpmdb_meta'][ WPMDB_CORE_SLUG ]['version'];
		}
		$diagnostic_info['mdb-plugins'] = $mdb_plugins;
		$curl_info                      = function_exists( 'curl_version' ) ? curl_version() : [];
		$diagnostic_info['server-info'] = array(
			...
			'Set Time Limit Available'        => ( function_exists( 'set_time_limit' ) ) ? 'Yes' : 'No',
			'Blocked External HTTP Requests'  => ( defined( 'WP_HTTP_BLOCK_EXTERNAL' ) && WP_HTTP_BLOCK_EXTERNAL ) ? ( defined( 'WP_ACCESSIBLE_HOSTS' ) && ! empty( WP_ACCESSIBLE_HOSTS ) ? 'Partially (Accessible Hosts: ' . WP_ACCESSIBLE_HOSTS . ')' : 'All' ) : 'None',
			...
			'cURL Version'                    => isset( $curl_info['version'] ) ? $curl_info['version'] : 'Unknown',
			...
			// phpcs:enable
			'Basic Auth'                      => Util::basic_auth_enabled() ? 'Enabled' : 'Disabled',
		);

		$diagnostic_info['db-server-info'] = array(
			'MySQL'                    => Util::get_db_server_info(),
			'ext/mysqli'               => empty( $wpdb->use_mysqli ) ? 'no' : 'yes',
			'WP Locale'                => get_locale(),
			'DB Charset'               => defined( 'DB_CHARSET' ) ? DB_CHARSET : 'Not Defined',
			// Note that we use get_option here, not get_bloginfo, as get_bloginfo( 'charset' ) will return 'UTF-8' by default.
			'blog_charset'             => get_option( 'blog_charset', 'Unknown' ),
		...
		$diagnostic_info = array_merge(
			$diagnostic_info,
			apply_filters( 'wpmdb_diagnostic_info', array(), $diagnostic_info )
		);
		...
		if ( is_multisite() ) {
			...
			$active_plugins = array_map(
				array( $this->util, 'remove_wp_plugin_dir' ),
		...
		foreach ( $active_plugins as $plugin ) {
			...
			$active_plugins_log[1][] = $this->util->get_plugin_details(
				WP_PLUGIN_DIR . '/' . $plugin,
				isset( $whitelist[ $plugin ] ) ? '*' : ''
			);
		}
	...
	/**
	 * Automatically log migration error
	 *
	 * @param array $error
	 *
	 * @return void
	 ...
	 */
	public function automatically_log_migration_error( $error ) {
