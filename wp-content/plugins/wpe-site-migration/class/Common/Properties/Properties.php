...
class Properties {
	...
	public function __construct() {
		...
		$this->invalid_content_verification_error = __(
			'The connection information could not be authenticated because it is incorrect or expired. Copy new connection information from the Settings tab of the remote site and try again.',
			'wp-migrate-db'
		);

		$this->plugin_file_path = realpath( dirname( __DIR__ ) . $this->set_plugin_file_path() );
		...
		$this->plugin_title       = WPMDB_PLUGIN_TITLE;
		...
		$this->core_slug = WPMDB_CORE_SLUG;
		$this->is_pro    = $is_pro;
	...
	/**
	 * Sets plugin file path for the plugin
	 *
	 * @return String
	 **/
	public function set_plugin_file_path() {
		switch ( Util::appEnv() ) {
			case 'free':
				return '/../../wp-migrate-db.php';
			case 'wpe':
				return '/../../wpe-site-migration.php';
			default:
				return '/../../wp-migrate-db-pro.php';
		}
	}
}
