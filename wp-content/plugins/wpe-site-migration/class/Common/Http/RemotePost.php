...
class RemotePost extends Http {
	...
	public function post( $url, $data, $scope, $args = [], $expecting_serial = false ) {
		...
		if (
			...
			0 === strpos( $url, 'https://' ) &&
			'ajax_verify_connection_to_remote_site' === $scope
		) {
		...
		$response_status = $this->handle_remote_post_responses(
			$response,
			$url,
			$scope,
			$expecting_serial,
		...
		if ( true !== $response_status ) {
			return $this->handle_response_code(
				$response_status,
				$response,
				$url,
				$data,
		...
		switch ( $code ) {
			...
			case self::RESPONSE_UNEXPECTED_ERROR:
				return new WP_Error(
					...
					sprintf(
						__(
							'The connection failed, an unexpected error occurred, please contact support. (#121 - scope: %s)',
							'wp-migrate-db'
						),
						$scope
					)
			...
			case self::RESPONSE_TIMED_OUT:
				return new WP_Error(
					...
					sprintf(
						__(
							'The connection to the remote server has timed out, no changes have been committed. (#134 - scope: %s)',
							'wp-migrate-db'
						),
						$scope
					)
			...
			case self::RESPONSE_443_ERROR:
				return new WP_Error(
					...
					sprintf(
						__(
							'Couldn\'t connect over HTTPS. You might want to try regular HTTP instead. (#121 - scope: %s)',
							'wp-migrate-db'
						),
						$scope
					)
			...
			case self::RESPONSE_SSL_ERROR:
				return new WP_Error(
					...
					sprintf(
						__(
							'<strong>HTTPS Connection Error:</strong>  (#121 - scope: %s) This typically means that the version of OpenSSL that your local site is using to connect to the remote is incompatible or, more likely, being rejected by the remote server because it\'s insecure. <a href="%s" target="_blank">See our documentation</a> for possible solutions.',
							'wp-migrate-db'
						),
						$scope,
			...
			case self::RESPONSE_401_ERROR:
				return new WP_Error(
					...
					__(
						'The remote site is protected with Basic Authentication. Please enter the username and password above to continue. (401 Unauthorized)',
						'wp-migrate-db'
					)
				);
			case self::RESPONSE_500_ERROR:
				return new WP_Error(
					...
					sprintf(
						__(
							'Unable to connect to the remote server, the remote server responded with: %s %s (scope: %s)',
							'wp-migrate-db'
						),
						$response_code,
						$message,
			...
			case self::RESPONSE_STATUS_ERROR:
				return new WP_Error(
					...
					sprintf(
						__(
							'Unable to connect to the remote server, please check the connection details - %1$s %2$s (#129 - scope: %3$s)',
							'wp-migrate-db'
						),
						$response_code,
						$message,
			...
			case self::RESPONSE_MDB_INACTIVE:
				return new WP_Error(
					...
					sprintf(
						__(
							'WP Migrate does not seem to be installed or active on the remote site. (#131 - scope: %s)',
							'wp-migrate-db'
						),
						$scope
					)
			...
			case self::RESPONSE_EMPTY_RESPONSE:
				return new WP_Error(
					...
					sprintf(
						__(
							'A response was expected from the remote, instead we got nothing. (#146 - scope: %1$s) Please review %2$s for possible solutions.',
							'wp-migrate-db'
						),
						$scope,
						sprintf(
							'<a href="%s" target="_blank">%s</a>',
							'https://deliciousbrains.com/wp-migrate-db-pro/doc/a-response-was-expected-from-the-remote/?utm_campaign=error%2Bmessages&utm_source=MDB%2BPaid&utm_medium=insideplugin',
							__( 'our documentation', 'wp-migrate-db' )
						)
					)
				);
	...
	protected function handle_block_external( $url ) {
		if ( defined( 'WP_HTTP_BLOCK_EXTERNAL' ) && WP_HTTP_BLOCK_EXTERNAL ) {
			...
			if (
				...
				( defined( 'WP_ACCESSIBLE_HOSTS' ) && ! in_array( $host, explode( ',', WP_ACCESSIBLE_HOSTS ) ) )
			) {
				return true;
			}
		}
	...
	protected function handle_unresolvable_host( $url ) {
		...
		if ( strstr( $url, 'dev.' ) || strstr( $url, '.dev' ) || ! strstr( $url_bits['host'], '.' ) ) {
			...
			if ( 'pull' === $this->state_data['intent'] ) {
				$error .= __(
					'It appears that you might be trying to pull from a local environment. This will not work if <u>this</u> website happens to be located on a remote server, it would be impossible for this server to contact your local environment.',
					'wp-migrate-db'
				);
			} else {
				$error .= __(
					'It appears that you might be trying to push to a local environment. This will not work if <u>this</u> website happens to be located on a remote server, it would be impossible for this server to contact your local environment.',
					'wp-migrate-db'
				);
			}
		}
