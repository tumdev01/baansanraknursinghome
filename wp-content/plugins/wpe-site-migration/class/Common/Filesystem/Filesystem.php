...
class Filesystem {
	 ...
	 * Recursive mkdir
	 *
	 * Note that if you pass a file path to this then it will create the filename
	 * as a directory. e.g. /tmp/file.txt/ <- will be a directory.
	 *
	 * @param string $abs_path
	 * @param int    $perms
	 *
	...
	public function get_file_info( $entry, $abs_path, $symlink = false, $stage = '' ) {
		...
		$return['wp_content_path']    = str_replace( $this->slash_one_direction( $content_path ) . DIRECTORY_SEPARATOR,
			'',
			$full_path );
		...
		$return['folder_name'] = count( $exploded ) > 2 ? $exploded[1] : '.';
	...
	/**
	 * Copy a file. Does not work with a directory as the source.
	 *
	...
	public function copy( $source_abs_path, $destination_abs_path, $overwrite = true, $perms = false ) {
		// Return the error state if source file doesn't exist or if the source is a directory.
		if ( ! $this->file_exists( $source_abs_path ) || is_dir( $source_abs_path ) ) {
			return false;
		}
		...
		// If copying a directory, prepare the destination.
		if ( is_dir( $source_abs_path ) && ! is_dir( $destination_abs_path ) ) {
			$this->mkdir( $destination_abs_path );
		}
		...
		if ( ! $return && $this->use_filesystem ) {
			...
			$return               = $this->wp_filesystem->copy( $source_abs_path,
	...
	public function move( $source_abs_path, $destination_abs_path, $overwrite = true ) {
		// Return the error state if source file doesn't exist.
		if ( ! $this->file_exists( $source_abs_path ) ) {
			return false;
		}

		// Recursively create the destination's directory before we start.
		$this->ensure_path_exists( $destination_abs_path );
		...
		if ( ! $overwrite && $this->file_exists( $destination_abs_path ) ) {
			...
			// Note that $this->copy() does not work for directories.
			if (
				! is_dir( $source_abs_path ) &&
				$this->copy( $source_abs_path, $destination_abs_path, $overwrite ) &&
				$this->file_exists( $destination_abs_path )
			) {
	...
	/**
	 * Ensures that the path to a file exists by recursively creating any
	 * directories that don't exist.
	 *
	 * The difference between this and mkdir is that if you pass a file path to
	 * mkdir it will create the filename as a directory. e.g.
	 * /tmp/file.txt/ <- will be a directory.
	 *
	 * @param string $absolute_path
	 *
	 * @return void
	 */
	public function ensure_path_exists( $absolute_path ) {
		$path = pathinfo( $absolute_path, PATHINFO_DIRNAME );
		$this->mkdir( $path );
	}
	...
	/**
	 * Initiate a file download for the user.
	 *
	 * @return void
	...
	public function download_file() {
		...
		if ( $is_full_site_export ) {
			do_action( 'wpmdb_migration_complete', 'savefile', '' );
		}
		...
		if ( is_wp_error( $dump_name ) ) {
			wp_die( sprintf(
				__( '<h3>Could not get filename to download. </h3> %s', 'wp-migrate-db' ),
				$dump_name->get_error_message()
			) );
		}

		$diskfile = self::get_upload_info( 'path' ) . DIRECTORY_SEPARATOR . $dump_name;
		if ( $is_full_site_export ) {
			$diskfile = self::get_upload_info( 'path' ) . DIRECTORY_SEPARATOR . $raw_dump_name . '.zip';
		}
	 ...
	 */
	public static function get_upload_info( $type = 'path' ) {
	...
	public function get_backups() {
		$backup_dir = self::get_upload_info( 'path' ) . DIRECTORY_SEPARATOR;
	...
	public function get_local_plugins( $exclude_mdb = true ) {
		...
		$plugin_list    = array();
		$plugins        = Files_Util::filter_excluded_plugins( $plugins, $exclude_mdb );

		foreach ( $plugins as $key => $plugin ) {
			$base_folder = preg_replace( '/\/(.*)\.php/i', '', $key );

			$plugin_path         = array_key_exists( $base_folder,
				$plugin_paths ) ? $plugin_paths[ $base_folder ] : false;
	...
	/**
	 * Returns true if our operating system will correctly give us
	 * file permissions.
	 *
	 * @return bool
	 */
	public static function can_get_file_permissions() {
		return ! Util::is_windows();
	}

	/**
	 * Gets a file's permissions as an octal string.
	 *
	 * Returns an empty string if permissions can't be determined.
	 *
	 * @param $file
	 * @return string
	 */
	public function fileperms_as_string( $file ) {
		if ( ! $this->can_get_file_permissions() ) {
			return '';
		}
		...
		// Code copied from PHPs fileperms docs: https://www.php.net/manual/en/function.fileperms.php
