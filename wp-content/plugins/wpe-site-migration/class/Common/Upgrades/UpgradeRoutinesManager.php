...
class UpgradeRoutinesManager {
	...
	public function perform_upgrade_routines() {
		...
		foreach ( [ WPMDB_SAVED_PROFILES_OPTION, WPMDB_RECENT_MIGRATIONS_OPTION ] as $profile_type ) {
			...
			foreach ( $profiles as $profile ) {
				...
				$profile      = $this->profile_manager->get_profile_by_id(
				...
				foreach ( $this->get_routines() as $routine ) {
					...
					if (
						version_compare(
							$routine->get_target_schema_version(),
			...
			if ( ! empty( $last_routine_version ) ) {
				...
				if ( false === $updated_profiles ) {
					error_log(
						sprintf(
							'WPMDB: Could not update %s to schema version: %s. It might already be up-to-date.',
							$profile_type,
							$last_routine_version
						)
					);
				}
			}
		...
		if (
			( ! empty( $last_routine_version ) && version_compare( $last_routine_version, $target_version ) === 0 ) ||
			"0" === $this->get_current_schema_version()
		) {
	...
	private function get_current_schema_version() {
		return (string) get_site_option( WPMDB_SCHEMA_VERSION_OPTION, 0 );
	}
	...
	private function update_schema_version( $version = "0" ) {
		update_site_option( WPMDB_SCHEMA_VERSION_OPTION, $version );
	}
}
