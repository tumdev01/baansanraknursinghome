...
class Payload {
	 ...
	 * not already exist.
	 *
	 * Raises exceptions if the directory or file exist and are not writable,
	 * or if an error occurs when opening the file.
	 *
	 * @param array $state_data
	 * @param array $meta
	 *
	 * @return array  An array of 'dest' (the full filepath) and 'handle' (the file handle)
	 * @throws Exception
	 */
	public function get_handle_from_metadata( $state_data, $meta ) {
		...
		if ( ! is_dir( $dirname ) ) {
			$this->create_directory( $dirname );
		}

		$this->check_directory_is_writable( $dirname, $dest );

		$this->check_file_is_writable( $dest );
	...
	public function check_directory_is_writable( $dirname, $dest ) {
		if ( is_dir( $dirname ) && ! is_writable( $dirname ) ) {
			if ( $this->filesystem->can_get_file_permissions() ) {
				$message = sprintf( __( 'The parent folder of the file `%s` is not writable. Folder permissions are %s. Please ensure the web server can read from and write to this file\'s parent folder/directory.',
					'wp-migrate-db' ),
					$dest,
					$this->filesystem->fileperms_as_string( $dirname )
				);
			} else {
				$message = sprintf( __( 'The parent folder of the file `%s` is not writable. Please ensure the web server can read from and write to this file\'s parent folder/directory.',
					'wp-migrate-db' ),
					$dest );
			}
